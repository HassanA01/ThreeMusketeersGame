...
import assignment1.Exceptions.InvalidResponseException;
...
public class ThreeMusketeers {
    ...
    private BuyGuardOrder buyGuardOrder;		// Needs to be instantiated if player chooses challenge mode.
    private SellCreditCommand sellCreditCommand;
    ...
    public ThreeMusketeers(String boardFilePath) {
        this.board = new Board(boardFilePath, this);
    }
    ...
    public Piece.Type getHumanPlayerType(){
    	if (musketeerAgent instanceof HumanAgent) return Piece.Type.MUSKETEER;    	
    ...
    public void play(GameMode mode){
        selectMode(mode);
    	choosePlayWithMovesCounter(mode);
        runGame();
    }
    
    private void choosePlayWithMovesCounter(GameMode mode) {
    	
    	if (mode != GameMode.Human) {
    		System.out.println("Would you like to play with a move counter(y/n): ");
    		String response = scanner.next();

    		try {
    	   		if (!(response.equals("y") || response.equals("n"))) {
        			throw new InvalidResponseException("You must choose 'y' or 'n'!");
        		}        	
        		else if (response.equals("y")) MovesCounter.instance = new RealMovesCounter(chooseMoveCount());
        		else MovesCounter.instance = new NullMovesCounter();
    		}
    		...
    		MovesCounter.instance = new NullMovesCounter();
    	}
    }
    
    private int chooseMoveCount() {
    	System.out.println("How many moves do you want: ");
    	try {
    		int response = Integer.valueOf(scanner.next());
    		if (response < 1) 
    			throw new InvalidResponseException("You must enter an integer greater than 0!");
    		
    		return response;
    	} 
    	catch(InvalidResponseException e){
    		System.out.println(e.getMessage());
    		return chooseMoveCount();
    	}
    	catch(Exception e) {
    ...
    private void runGame() {
        while(!board.isGameOver()) {
            ...
            if (currentAgent instanceof HumanAgent) // Human move
                ...
                move(currentAgent);
                MovesCounter.instance.decrementMoves();
            }
        }
    ...
    public void sellCredit() {
    	...
    	orderQueue.placeOrder(buyGuardOrder);

    }
    
    public void buyPiece() {
    	
    	orderQueue.placeOrder(sellCreditCommand);
