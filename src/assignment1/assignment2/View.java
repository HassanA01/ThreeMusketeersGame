...
import assignment2.ThreeMusketeers.GameMode;
...
public class View {
    ...
    protected void runMove() { // TODO
    	...
    	if (!(gameMode == GameMode.Human) && !(model.isHumanTurn())) {
    	...
    	if (model.getBoard().isGameOver()) {
    		...
    		setMessageLabel(model.getBoard().getWinner().getType() + " WINS!");
    	}
    ...
    private void saveBoard() { // TODO
    	...
    	else if (new File("boards", saveFileNameTextField.getText()).exists()) {
    		
    		saveFileErrorLabel.setText(saveFileExistsError);
    	}
    	
    	else {
    		model.getBoard().saveBoard(new File("boards", saveFileNameTextField.getText()));
